#!/usr/bin/env python3
"""Generate synthetic benchmark queries for RAG evaluation (BLIND mode).

For each chunk in the pack JSONs, generates 2-3 realistic emergency queries
using ONLY the topic descriptor (packName + sectionPath), NOT the full content.
This produces "blind" queries that test retrieval without answer leakage.
...
Usage:
    ...
    python tools/rag_eval/generate_dataset_blind.py \
        --packs assets/rag/packs \
        --output benchmark/dataset_blind.json
...
def _build_prompt(pack_name: str, section_path: str) -> str:
    """Build prompt using ONLY topic descriptor, NOT chunk content."""
    return f"""Given this emergency topic (pack name + section path), generate 2-3 realistic questions a person in an emergency might ask.

IMPORTANT: You DO NOT see the full content — only the topic. Generate plausible questions based on what someone would likely need to know about this topic.

Requirements:
- Urgent, colloquial phrasing (e.g. 'how do I stop bleeding')
- Questions relevant to this specific topic area
- Vary phrasing (direct questions, descriptive situations, short keywords)

Topic: '{pack_name}' — {section_path}

Respond in JSON: {{"queries": ["q1", "q2", "q3"]}}"""


def load_packs(packs_dir: str) -> list[dict]:
    """Load all pack JSONs and return flat list of chunk metadata.

    NOTE: Does NOT load chunk text — only pack_id, pack_name, chunk_id, and section_path.
    """
...
async def generate_queries_for_chunk(
    ...
    """Generate queries for a single chunk via Claude API (BLIND mode)."""
...
async def main_async(args):
    ...
    print(f"Mode:     BLIND (topic-only, no content)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate synthetic benchmark queries (BLIND mode - topic only)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output", default="benchmark/dataset_blind.json",
        help="Output path for blind benchmark dataset"
    )
